...
export async function createFilm(req, res) {
  try {
    if (!req.validatedData) {
      return res.status(400).json({ message: "Données de validation manquantes (fichiers/liens)." });
    }

    const { duration, poster, video } = req.validatedData;
    ...
    await handleFileProcessing(newFilm.id, req.files, null, req.body);
    ...
    if (film && film.User) {
      try {
        ...
        if (req.files?.film?.[0]) fs.unlinkSync(req.files.film[0].path);
        if (req.files?.poster?.[0]) fs.unlinkSync(req.files.poster[0].path);
...
export const getYoutubeInfo = async (req, res) => {
  ...
  if (!videoId) return res.status(400).json({ message: "URL de YouTube no válida" });

  const API_KEY = process.env.YOUTUBE_API_KEY;
  const apiUrl = `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,status&id=${videoId}&key=${API_KEY}`;
  ...
  try {
    ...
    // Duracion
    const durationSeconds = controlDuration(contentDetails.duration);


    if (durationSeconds > 120) {
      return res.status(400).json({
        message: `El video es demasiado largo (${durationSeconds}s). Máximo 120 segundos.`
      });
    }

    if (!status.embeddable) {
      return res.status(400).json({ message: "El video no permite integración (Copyright)." });
    }
    ...
    res.status(500).json({ message: "Error al conectar con la API de YouTube" });
  }
};
...
export async function updateFilm(req, res) {
  try {
    ...
    await handleFileProcessing(film.id, req.files, existingFileRecord, req.body);
