...
const AdminPending = () => {
    ...
    if (loading) {
        ...
        tableContent = filteredFilms.map((film) => {
            ...
            const directorName = film.User ? `${film.User.firstName} ${film.User.lastName}`.trim() : "NON RENSEIGNÉ";

            return (
                <tr key={film.id} className="group hover:bg-gray-50/50 transition-colors">
                    <td className="py-5 pl-4">
                        <div className="w-14 h-9 overflow-hidden rounded-lg bg-gray-100 border border-gray-50">
                            <img src={poster} className="h-full w-full object-cover" alt="poster" onError={(e) => { e.target.src = "/src/assets/youtube.png"; }} />
                        </div>
                    </td>
                    <td className="py-5">
                        <p className="font-black text-[11px] text-black uppercase">{film.title || "SANS TITRE"}</p>
                    </td>
                    <td className="py-5 text-[10px] font-bold text-gray-800 uppercase">{directorName}</td>
            ...
            <main className="flex-1 p-8 lg:p-12 overflow-y-auto">
                ...
                <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden p-6">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-50">{tableContent}</tbody>
                    </table>
                </div>
            </main>
        </div>
