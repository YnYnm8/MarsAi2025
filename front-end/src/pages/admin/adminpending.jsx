...
const AdminPending = () => {
    ...
    const fetchFilmsToDiscuss = async () => {
        try {
            const response = await fetch('http://localhost:3000/admin/films/pending', {
                credentials: 'include'
            });
            ...
            console.error("Erreur chargement films en attente:", err);
        } finally {
    ...
    // Ouvre la modale avec la configuration adaptée
        ...
        <div className="flex min-h-screen bg-[#F2F2F2] font-sans text-left relative">
            ...
            <main className="flex-1 p-8 lg:p-12 overflow-y-auto">
                <div className="mb-12">
                    ...
                    <h1 className="text-4xl font-black text-black mb-4 tracking-tighter uppercase">FILMS À DISCUTER</h1>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-[10px] text-gray-400 uppercase tracking-widest border-b border-gray-50">
                                ...
                                <th className="pb-6 text-center">Décision</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                [...Array(5)].map((_, i) => (
                                    <tr key={i}><td colSpan="7" className="py-4"><div className="animate-pulse bg-gray-200 h-12 w-full rounded-xl"></div></td></tr>
                                ))
                    ...
                    {!loading && filteredFilms.length > 0 && (
                        <div className="mt-8 flex items-center justify-between px-4 py-4 border-t border-gray-50">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 text-[10px] font-black uppercase rounded-xl transition-all disabled:text-gray-200">Précédent</button>
                                <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 text-[10px] font-black uppercase rounded-xl transition-all disabled:text-gray-200">Suivant</button>
                            </div>
                        </div>
                    )}
                </div>
