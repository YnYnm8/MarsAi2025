...
const AdminRejected = () => {
    ...
    const fetchRejected = async () => {
        try {
            const response = await fetch(`${VITE_API_URL_FRONTEND}/admin/films/rejected`, { credentials: 'include' });
        ...
        } catch (err) { console.error("Erreur chargement films refusés:", err); }
    ...
    const openModal = (id) => { setSelectedFilmId(id); setIsModalOpen(true); };
    ...
    const confirmMoveToPending = async () => {
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                setIsModalOpen(false); setSelectedFilmId(null);
            } else alert("Erreur serveur : " + (result.message || "Action impossible"));
    ...
    const PaginationBar = () => (
        <div className="flex items-center justify-between px-2 py-4 border-t border-gray-50">
            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest hidden sm:block">{indexOfFirstFilm + 1} - {Math.min(indexOfLastFilm, filteredFilms.length)} SUR {filteredFilms.length}</div>
            <div className="text-[10px] font-bold text-gray-400 uppercase sm:hidden">{currentPage}/{totalPages}</div>
            <div className="flex gap-2">
                <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1} className={`px-4 py-2 text-[10px] font-black uppercase rounded-xl ${currentPage === 1 ? 'text-gray-200' : 'text-black hover:bg-gray-100'}`}>Préc.</button>
                <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className={`px-4 py-2 text-[10px] font-black uppercase rounded-xl ${(currentPage === totalPages || totalPages === 0) ? 'text-gray-200' : 'text-black hover:bg-gray-100'}`}>Suiv.</button>
            </div>
        </div>
    );
        ...
        <div className="flex min-h-screen bg-[#F2F2F2] font-sans text-left relative">
            ...
            <main className="flex-1 p-4 pt-4 pb-20 lg:pt-8 lg:pb-0 lg:p-12 overflow-y-auto">
                <div className="mb-8">
                    ...
                    <h1 className="text-2xl lg:text-4xl font-black text-black mb-4 tracking-tighter uppercase">FILMS REFUSÉS</h1>
                </div>
                <div className="mb-6">
                    <input type="text" placeholder="Rechercher dans les refusés..."
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-[10px] text-gray-400 uppercase tracking-widest border-b border-gray-50">
                                ...
                                <th className="pb-6 text-center">RÉEXAMINER</th><th className="pb-6 text-right pr-4">Détails</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? [...Array(5)].map((_, i) => (
                                ...
                                return (
                                    <tr key={film.id} className="group hover:bg-gray-50/50 transition-colors">
                                        <td className="py-5 pl-4"><div className="w-14 h-9 overflow-hidden rounded-lg bg-gray-100 shadow-sm border border-gray-50"><img src={getPoster(film)} className="h-full w-full object-cover" alt="poster" onError={(e) => { e.target.src = "/src/assets/youtube.png"; }} /></div></td>
                ...
                <div className="lg:hidden space-y-3">
                    {loading ? [...Array(5)].map((_, i) => <Skeleton key={i} />) :
                        filteredFilms.length === 0 ? (
                            <div className="py-10 text-center text-gray-400 text-sm italic bg-white rounded-2xl">Aucun film refusé trouvé</div>
                        ) : currentFilms.map((film) => {
                            const { label, classes } = getStatusDetails(film.status);
                            return (
                                <div key={film.id} className="bg-white rounded-2xl p-4 shadow-sm">
                                    ...
                                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                                        ...
                                        <button onClick={() => openModal(film.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 text-blue-600 text-[9px] font-black uppercase hover:bg-blue-100 transition-colors">
                                            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                            Réexaminer
                                        </button>
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
                    <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl text-center">
                        <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-blue-50 mb-6">
                            <svg className="h-8 w-8 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                        </div>
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={confirmMoveToPending} className="flex-1 py-4 px-6 rounded-2xl text-[10px] font-black uppercase bg-black text-white hover:bg-gray-900 transition-all">Confirmer</button>
                        </div>
                    </div>
                </div>
            )}
