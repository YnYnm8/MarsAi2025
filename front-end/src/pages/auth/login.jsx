...
const Login = () => {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (!response.ok) {
        setServerError(
          t(data.message, { defaultValue: t("error_login_failed") })
        );
        return;
      }

      // ✅ Toujours stocker le user dans le context en premier
      setUser(data.user);
      ...
      <div className="min-h-screen flex flex-col items-center px-6 pt-24 pb-12 relative z-10">
        ...
        <div className="w-full max-w-sm bg-[#111] border border-gray-800 rounded-2xl shadow-2xl shadow-purple-900/10 px-6 py-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <div className={`flex items-center bg-black rounded-xl px-4 py-3 border transition-colors ${errors.email ? "border-red-500" : "border-gray-700 focus-within:border-blue-500"}`}>
                <img src={mailIcon} alt="logoMail" className="h-5 w-auto object-contain mr-3 invert opacity-70" />
                <input
                  type="email"
              ...
              {errors.email && (
                <p className="text-red-500 text-[10px] mt-1 font-bold">{t(errors.email.message)}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className={`flex items-center bg-black rounded-xl px-4 py-3 border transition-colors ${errors.password ? "border-red-500" : "border-gray-700 focus-within:border-blue-500"}`}>
                <img src={cadenaIcon} alt="logoCadena" className="h-5 w-auto object-contain mr-3 invert opacity-70" />
                <input
                  type="password"
                  placeholder="••••••••••"
                  className="bg-transparent w-full outline-none text-sm text-white placeholder-gray-600 font-medium"
              ...
              {errors.password && (
                <p className="text-red-500 text-[10px] mt-1 font-bold">{t(errors.password.message)}</p>
              )}
            </div>
          ...
          <p className="text-center text-xs text-gray-500 tracking-wide">
            {t("new_user")}{" "}
            <Link to="/register" className="font-bold text-gray-300 hover:text-white transition-colors ml-1">
              {t("register_link")}
            </Link>
          </p>
        </div>
